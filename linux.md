...
```bash
# validate if the content of a document is a legal json string + pretty format
...
BusyBox combines tiny versions of many common UNIX utilities into a single small executable. Since it provides binary download, it can be used on Unix/Linux based systems which do not support package installation (scp busybox onto them and run directly).

Busybox ships with a large num. of applets (refer to [its document](https://busybox.net/downloads/BusyBox.html) for details). Below is an example how to use busybox as a HTTP server:
...
## List all available versions of a package with yum
...
EPEL stands for **Extra Packages for Enterprise Linux**, a.k.a repositories for extra packages, which contains lots of tools such as fio, ipvsadm, etc.
...
2. Search the package which is not in the official repositories and epel, say "fasd";
3. Select/click the project which is the best from the result list;
4. Click the "Repo Download" link based on the target release;
5. Copy the URL field of the browser (not the content of the repo), say https://copr.fedorainfracloud.org/coprs/rdnetto/fasd/repo/fedora-33/rdnetto-fasd-fedora-33.repo for fasd;
6. sudo yum-config-manager --add-repo=\<the repo link just copied\>;
7. Check /etc/yum.repos.d/\<the newly created repo name\>.repo to make sure the contents generated is correct.
...
- dnf list --upgrades [expression]: list upgradable packages [matching expression]
