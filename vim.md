...
- `:help pattern.txt` - Search and replace used patterns, e.g. regular expression, escaping, etc.
...
- `:help function-list` - builtin functions of vim (`:help functions` also works)
...
- `:verbose <command>` - Show verbose information while running a command
...
- `Ctrl + y` - Insert without line break for auto completion
- `Ctrl + n/p` - Select the next/previous choice from the auto completion list
...
- `:map` - check existing map
  - `:map <C-e>` - checking the current key mapping for Ctrl + e
  - `:verbose map <C-e>` - checking the current key mapping for Ctrl + e with verbose information
...
- Commands:
  ...
  - `:sp <file>` - open file in a horizontally split window
  - `:vsp <file>` - open file in a vertically split window
  - `:new` - split window horizontally (editing a new/empty buffer)
  - `:vnew` - vsplit window vertically (editing a new/empty buffer)

- Split with an existing buffer
...
- `<C-w>T` - move the split window as a tabview (another way to maximize window)
...
- `<C-w>|` - maximize current vertically split window
- `<C-w>_` - maximize current horizontally split window
...
Some plugins may lead to vim slow response. Profiling can help identify the culprit.
...
```
# vim.log will record all debug info with verbose level 9, default is 10
