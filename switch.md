...
One initiator + multiple targets is the best practice of configuring zones. However, it is tedious to configure multiple hosts with the same targets, since an end user needs to create num. of hosts x zones, which share the identical targets info. User errors is easy to be involved at the same time.
...
2. Check available domain ID(the switch used as upstream does not need to change its domain id, the downstream switch need to change its domain id to avoid conflict)
3. From the downstream switch:
...
- Show ISL: after enabling virtual fabrics, actual ISL can only be shown on the base switch(logical switch do not contain real physical connection info)
...
- config
- zoneset activate name \<Name\> vsan \<X\>
- exit
- config
- zone commit vsan \<X\>
- exit
...
- Decode FCID: Domain ID(1 byte) + Area ID(1 byte) + Port ID(1 byte)
  ...
  …...
  - Locate Switch with Domain 0x22
