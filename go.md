...
go get -u ./...
# upgrade all dependencies, including test dependencies
go get -t -u ./...
# remove a dependency
...
- Basics
  ...
  [Github Reference](https://github.com/go-delve/delve)
...
```go
// for loop won't be stopped if break w/o using a label
...
for {
  ...
  case v, ok := <-in1:
    if !ok {
      in1 = nil // if in1 is closed, turn it off by assigning nil,
...
```go
# for type parameters/constrains examples:
